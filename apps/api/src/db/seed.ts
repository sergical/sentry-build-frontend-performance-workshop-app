...
async function seedSaleData(insertedProducts: any[]) {
  ...
  // Create a map for quick lookup (not currently used but useful for future optimization)
  const _productMap = new Map(insertedProducts.map((p) => [p.name, p]));

  for (const sale of saleMetadata) {
    // Find products matching this base name
    const matchingProducts = insertedProducts.filter(
      (p) => p.name === sale.name
    );
...
async function seed() {
  try {
    ...
    await pool.query(
      'TRUNCATE TABLE user_carts, purchases, product_metadata, sale_prices, products, sale_categories, users RESTART IDENTITY CASCADE'
    );
