...
router.post(
  '/retrieve',
  apiKeyAuth,
  async (req: BasicAuthRequest, res: Response) => {
    try {
      if (!req.user) {
      ...
      if (userId !== req.user.userId) {
        console.warn(
          `User ${req.user.userId} attempted to access payment methods for user ${userId}`
        );
      ...
      if (
        transactionContext.itemCount === undefined ||
        transactionContext.itemCount < 0
      ) {
        return res.status(400).json({
      ...
      console.log(
        `Retrieving payment methods for user ${userId}, amount: ${transactionContext.amount}`
      );
      ...
      const defaultPaymentMethod =
        userPaymentMethods.find((pm) => pm.isDefault) || userPaymentMethods[0];
      ...
      if (
        defaultPaymentMethod.expiryYear < currentYear ||
        (defaultPaymentMethod.expiryYear === currentYear &&
          defaultPaymentMethod.expiryMonth < currentMonth)
      ) {
      ...
      if (transactionContext.amount > 1000) {
        console.warn(
          `High-value transaction attempted by user ${userId}: $${transactionContext.amount}`
        );
        return res.status(403).json({
          error: 'Transaction requires additional verification',
          details:
            'High-value transactions must be processed with manual payment entry',
        });
      }

      console.log(
        `Successfully retrieved encrypted payment method for user ${userId}`
      );
...
router.post(
  ...
  async (req: BasicAuthRequest, res: Response) => {
    try {
      ...
      const errorMessage =
        (error as Error)?.message || 'Unknown decryption error';
      console.error(
