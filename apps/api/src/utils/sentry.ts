...
if (!WEBHOOK_SECRET) {
  throw new Error(
    'FATAL ERROR: FEATURE_FLAG_WEBHOOK_SECRET is not defined in the environment.'
  );
}

// Ensure Sentry webhook URL ends with a trailing slash
const ensuredSentryUrl =
  SENTRY_WEBHOOK_URL && !SENTRY_WEBHOOK_URL.endsWith('/')
    ? `${SENTRY_WEBHOOK_URL}/`
    : SENTRY_WEBHOOK_URL;
...
export async function sendSentryNotification(
  flagName: string,
  action: string,
  userId?: string,
  userType?: string
  ...
  try {
    ...
    if (!sentryResponse.ok) {
      ...
      console.error(
        `❌ Sentry webhook notification failed for ${flagName}: ${sentryResponse.status}`
      );
    ...
    console.log(
      `✅ Sentry notification sent successfully for ${flagName} (${action}).`
    );
    console.log(`✅ Response body: ${responseBody}`);
    return true;
  } catch (error) {
    console.error(
      `❌ Error sending Sentry notification for ${flagName}:`,
      error
    );
    return false;
  } finally {
