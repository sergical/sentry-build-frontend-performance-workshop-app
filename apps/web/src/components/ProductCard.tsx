...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    if (user) {
      console.log(
        `User '${user.username}' added item '${product.name}' (ID: ${product.id}) to cart.`
      );
    } else {
      console.log(
        `Guest added item '${product.name}' (ID: ${product.id}) to cart.`
      );
    }

    console.log(
      `Adding product ID: ${product.id} (Name: ${product.name}) to cart from ProductCard`
    );
    dispatch({
      type: 'ADD_ITEM',
