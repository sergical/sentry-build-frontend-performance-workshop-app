...
export async function GetPaymentDetails(
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `Payment vault returned ${response.status}: ${response.statusText}`
      );
      return null;
    }
    ...
    if (vaultData.success && vaultData.encryptedPaymentData) {
      ...
      try {
        ...
        const decryptionResponse = await fetch(
          `${API_BASE_URL}/api/payment-vault/decrypt`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        if (!decryptionResponse.ok) {
          throw new Error(
            `Decryption service failed: ${decryptionResponse.statusText}`
          );
        }
